export const lab5 = {
  ...
  intro: `A real security system doesn't copy and paste its scoring logic everywhere.
...
def greet_analyst(__BLANK__):
    return f"Welcome, {__BLANK__}. System is online."
...
def analyze_login(fails, new_device):
    risk  = __BLANK__(fails, new_device)   # call score function
    label = __BLANK__(risk)                # call label function
...
for fails, nd in cases:
    ...
    print(f"fails={fails} nd={nd} :  risk={result[__BLANK__]} label={result[__BLANK__]}")`,
      hints: [
